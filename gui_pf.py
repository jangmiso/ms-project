...
conn = pymysql.connect(host='172.20.10.5', user='root', password='miso1004', db = 'ms_project', charset='utf8')
cur = conn.cursor()
...
class MainScreen2(QDialog):
    ...
    def show_sq_data(self):
        ...
        cur.execute("select 생도.중대, avg(점수.점수) from 점수, 수강, 과목, 생도 where 점수.수강번호=수강.수강번호 and 수강.과목코드=과목.과목코드 and 생도.교번=수강.교번 and 점수.최종여부=1 and 점수.날짜>='{}' and 점수.날짜<='{}' group by(생도.중대)".format(s_date, e_date))
        data1 = cur.fetchall()
        ...
        if data1==():
            ...
            colors=['C3','C1','yellow','C9','C0','C4','C6','C5','C7']
            self.MplWidget_3.canvas.axes.clear()
            ...
            for rect in bar:
                ...
                self.MplWidget_3.canvas.axes.text(rect.get_x()+rect.get_width()/2.0, height, '%.2f' % height, ha='center', va='bottom', size=12)
    ...
    def show_view_data(self):
        rank = self.comboBox_11.currentText()[:-1]
        sq = self.comboBox_13.currentText()[:-2]
        major = self.comboBox_14.currentText()
        cur.execute("select 점수.날짜, 생도.기수, 생도.중대, 생도.이름, 생도.전공, 점수.점수 from 점수, 수강, 과목, 생도 where 점수.수강번호=수강.수강번호 and 수강.과목코드=과목.과목코드 and 생도.교번=수강.교번 and 생도.중대={} and 생도.기수={} and 생도.전공='{}' and 점수.최종여부=1 ORDER BY 점수.날짜 DESC, 생도.이름 DESC".format(sq,rank,major))
        data2 = cur.fetchall()
    ...
    def show_rank_view(self):
        ...
        if rank=='전':
            cur.execute("select 생도.기수, 생도.이름, 생도.전공, avg(점수.점수) from 점수, 수강, 생도 where 점수.수강번호=수강.수강번호 and 생도.교번=수강.교번 and 생도.중대={} and date_format(점수.날짜,'%Y-%m')='{}-{}' and 점수.최종여부=1 group by(생도.이름)".format(sq,year, month))

        
        else:
            cur.execute("select 생도.기수, 생도.이름, 생도.전공, avg(점수.점수) from 점수, 수강, 생도 where 점수.수강번호=수강.수강번호 and 생도.교번=수강.교번 and 생도.중대={} and 생도.기수={} and date_format(점수.날짜,'%Y-%m')='{}-{}' and 점수.최종여부=1 group by(생도.이름)".format(sq,rank,year, month))
        data3 = cur.fetchall()
